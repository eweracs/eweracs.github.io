...
Click an anchor to select it – the rest of the diagonal and tag overlays fade to grey so the links stand out; [[Shift]]-click to select several. With an anchor selected, **drag**: the anchor itself never moves; a halo follows the cursor and snaps onto the nearest **on-curve** node, and releasing links the anchor to it. An anchor links to **any number of nodes** – attaching is additive. Grab an existing link’s halo to drag it onto another node (**move** the link) or clear of any node (**remove** it). Press [[⌫]] with the anchor selected to clear all of its links at once. Hold [[⌥]] on any of these to mirror the change across compatible masters (anchor matched by name, node by index).

Anchors and diagonal selection are mutually exclusive: while a diagonal is selected the anchors grey out and stop responding to clicks, so a click lands on the diagonal’s own controls instead – handy when an anchor sits right over the flip-axis button. Click empty space to deselect the diagonal and the anchors are live again. (The [[⌘A]] cycle’s anchor phases are the exception, showing diagonals and anchors selected together.)
